...
-   **sensor**: abi (from OR\_ABI in filename)
-   **level**: L1B (from L1b in filename)
-   **sector**: Full-Disk (from RadF in filename)
-   **num\_expected**: 16 (GOES ABI has 16 channels per full-disk scan)
...
-   View available metadata configs:
    `from geoips_driver.interfaces import data_monitor_configs; print(data_monitor_configs.plugins)`
...
2.  **Add a second data monitor** watching a different directory (e.g.,
    `data/backup`)
3.  **Change the heartbeat interval** to 10 seconds and observe in
    Prometheus
4.  **Create a metadata configuration** for a different satellite (if
    you have the data)
...
\](<https://github.com/biosafetylvl5/geoips_driver/tree/main/examples/tutorials/01-file-watcher>)
