...
Replicas of one builder identifier share one durable queue bound to the
file-found fanout, so they compete for deliveries and each file goes to one of
them. Under the old topology each replica declared its own exclusive,
server-named queue and bound that to the fanout, so every replica received
every file: a second replica doubled the work with no error, and the pipeline
emitted two jobs for every input.

This module runs several real containers against one real broker, with files
arriving throughout a runtime scale-up and scale-down.

Three observations are needed together.
...
* A ledger on the data volume, one line appended per dispatcher execution.
  ...
  dispatch overwrites the first and the output directory looks the same either
  way. The ledger is the only record that can count executions.
* The dispatcher's own counters. A duplicate job never reaches the script:
  with ``files_per_job: 1`` the job identifier is the file path, both replicas
  mint the same identifier for the same file, and the dispatcher's
  consumer-side dedupe drops the repeat before executing anything. A
  ledger-only test reads exactly-once whether or not the topology is broken.
  ``courier_dispatcher_dedupe_skips_total`` sees the repeat.
* Each replica's own ``courier_job_builder_files_received_total``, scraped
  from that replica's metrics endpoint. The first two observations answer
  whether any file was handled twice. They stay green when the broker hands
  the whole batch to replica A while replica B sits attached and idle, which
  is how a queue with a single active consumer reads, and how a replica whose
  channel is never given a delivery reads. Splitting the count by replica
  measures that the replicas compete, and checks exactly-once at the point of
  delivery, upstream of the dedupe.

``files_per_job`` is 1 throughout. A builder that emits one job per file does
not accumulate, so replicating it needs no shared state and the scenario needs
no Redis (see
:meth:`courier.interfaces.job_builders.JobBuilder._check_replication_safety`).

Only a drained scale-down is covered. Both queues are empty before replica B
is stopped, so B holds nothing unacknowledged and the broker has nothing to
redeliver. Killing a replica mid-backlog, so that its unacknowledged
deliveries are redelivered to the survivor and absorbed by the dispatcher's
dedupe, is a different property; arranging it would mean stalling a builder
that returns to idle in milliseconds.
"""
...
# A backstop for five containers and three seeded phases.
...
#: Port the metrics endpoint listens on inside each container. The config below
#: sets it explicitly, so the scrape reads that same server.
METRICS_PORT = 9187
...
#: The replicated builder. Counter lookups below pin both labels, so a scrape
#: cannot answer with a different series.
...
def _files_received(pipeline: Pipeline, replica: str) -> float | None:
    ...
    The counter increments as each file-found message is parsed, before any
    job is built, so it measures delivery; everything downstream of it is
    deduplicated.

    A labelled counter is materialised on first use, so an absent series means
    either that the replica received nothing or that the scrape did not
    answer. The call site tells the two apart by summing the replicas against
    the expected count.
...
def _dedupe_evidence(
    ...
    Both counters come from the same exposition text. "No duplicates were
    skipped" is an assertion about an absent or zero series, which a misspelt
    name, a wrong port or a dead endpoint also satisfies; the consume counter
    read from the same text is the positive control that fails first.
...
def _settle(
    ...
    A snapshot taken the moment the expected entries appear may be read on the
    way to more, so counts in this module are taken after the ledger has held
    still with both queues empty.

    An entry landing inside the window restarts the window. The monitor holds
    a file in an in-process queue between the inotify event and the publish,
    and during that gap the broker is empty and the ledger unchanged, so a
    late arrival means the snapshot was taken early. The per-phase assertions
    count duplicates wherever the extra entry lands.
...
def test_replicas_of_one_builder_share_the_files_and_dispatch_each_once(
    ...
    Reverted check, run and observed: stopping replica B right after the
    two-consumer gate, before the batch is seeded, fails ``took_b > 0`` and
    leaves every other count green, since one replica dispatching six files
    once satisfies them all. A queue declared with
    ``x-single-active-consumer``, or a replica that binds and is never given a
    delivery, reads the same way while the broker still reports two consumers.

    Two src-side reverts were reasoned about and never run, so what follows is
    a prediction. Setting ``exclusive=True`` in ``_file_found_queue_config``
    (``src/courier/broker/kombu.py``) does not reproduce the old topology on
    its own, because the queue is shared: the second builder's declaration
    should draw a 405 RESOURCE_LOCKED and the test should fail at the first
    gate. Reproducing pre-#44 behaviour also needs the per-connection queue
    name, which should likewise fail at the first gate, because the shared
    queue that gate names keeps its preflight declaration and never gains a
    consumer.

    The counters cover the partial regression: both replicas consume the
    shared queue and some file still reaches both of them. The ledger cannot
    see that, because both replicas mint the same job identifier (the file
    path) and the dispatcher's dedupe drops the repeat before the script runs,
    so one line per file is written either way. The replicas' receipt counters
    then sum to more than the batch,
    ``courier_dispatcher_jobs_consumed_total`` exceeds the ledger, and
    ``courier_dispatcher_dedupe_skips_total`` rises off zero. That zero is
    asserted beside a positive control from the same scrape, so a misspelt
    metric name or an unreachable endpoint cannot read as "no duplicates".
    """
    config = build_config(
        pipeline.namespace,
        pipeline.broker,
    ...
    # Every container predeclares this queue during preflight, the
    # producer-only one included, so its existence says nothing about
    # attachment.
    pipeline.await_consumers(files_found, 1)

    # The monitor is edge-triggered inotify with no start-up scan, and the
    # broker gives no signal for "the watcher is watching". The warm-up arms
    ...
    # Only replica A has run so far, so its counter is the offset for every
    # later per-replica count. Reading it here, against a ledger count already
    # known, checks that the scrape is live and correctly labelled.
    ...
    # Set equality already holds: _await_dispatched polls until every seeded
    # file appears, and _phase keeps only entries from this phase. The count
    # below is what a duplicate delivery breaks.
    ...
    # Who did the work. Both scrapes are taken with the batch finished and
    # both queues empty, so the two numbers describe the same settled state
    # even though they are separate execs.
    ...
    # ``None`` means the skip path never ran: a labelled counter is not
    # materialised until its first increment.
