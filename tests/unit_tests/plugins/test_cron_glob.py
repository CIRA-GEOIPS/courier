...
class TestCronGlobConfig:
    ...
    def test_missing_glob_pattern_raises(self, tmp_path):
        with pytest.raises(ValidationError):
            CronGlobConfig.model_validate(
                {"path": str(tmp_path), "cron_expression": "* * * * *"}
            )

    def test_missing_cron_expression_raises(self, tmp_path):
        with pytest.raises(ValidationError):
            CronGlobConfig.model_validate(
...
class TestScanDirectory:
    ...
    def test_yields_matching_files(self, mock_service, tmp_data_dir):
    ...
    def test_skips_directories(self, mock_service, tmp_data_dir):
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir, glob_pattern="*"))
        files = list(plugin._scan_directory())
        assert all(f.file.is_file() for f in files)

    def test_skips_already_seen(self, mock_service, tmp_data_dir):
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir))
    ...
    def test_recursive_glob(self, mock_service, tmp_data_dir):
        plugin = CronGlob(
    ...
    def test_yields_file_objects_with_correct_hostname(self, mock_service, tmp_data_dir):
...
class TestLRUEviction:
    ...
    def test_seen_set_never_exceeds_cap(self, mock_service, tmp_path):
    ...
    def test_evicts_oldest_entry(self, mock_service, tmp_path):
    ...
    def test_evicted_file_is_re_emitted(self, mock_service, tmp_path):
    ...
    def test_cap_enforced_on_partial_consumption(self, mock_service, tmp_path):
...
class TestSeedSeen:
    ...
    def test_populates_seen_set(self, mock_service, tmp_data_dir):
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir))
        plugin._seed_seen()
        assert len(plugin._seen) == 2

    def test_seeded_files_are_not_emitted(self, mock_service, tmp_data_dir):
        """Files present at seed time must not appear in the following scan."""
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir))
        plugin._seed_seen()
        assert list(plugin._scan_directory()) == []

    def test_seed_respects_max_seen_files(self, mock_service, tmp_path):
...
class TestWaitUntil:
    ...
    def test_returns_true_when_stop_event_is_set(self, mock_service, tmp_data_dir):
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir))
        plugin._stop_event.set()
        target = datetime.now() + timedelta(hours=1)
        assert plugin._wait_until(target) is True

    def test_returns_false_when_target_is_in_the_past(self, mock_service, tmp_data_dir):
        plugin = CronGlob(mock_service, _make_config(tmp_data_dir))
...
class TestFindFile:
    ...
    def test_run_on_start_emits_immediately(self, mock_service, tmp_data_dir):
    ...
    def test_run_on_start_false_emits_nothing_before_first_tick(
        self, mock_service, tmp_data_dir
    ):
    ...
    def test_ignore_existing_suppresses_initial_files(self, mock_service, tmp_data_dir):
    ...
    def test_health_true_during_operation_false_after(self, mock_service, tmp_data_dir):
